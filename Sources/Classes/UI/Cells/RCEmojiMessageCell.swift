...
class RCEmojiMessageCell: RCMessageCell {
    ...
    class func size(_ indexPath: IndexPath?, messagesView: RCMessagesView?) -> CGSize {
        guard let rcmessage = messagesView?.rcmessage(indexPath) else { return CGSize(width: 0, height: 0) }
        ...
        return CGSize(width: CGFloat(fmaxf(Float(width), Float(RCMessages.emojiBubbleWidthMin()))),
                      height: CGFloat(fmaxf(Float(height), Float(RCMessages.emojiBubbleHeightMin()))))
    }
